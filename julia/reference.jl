...
NetworkReference(filename::String; kwargs...) = NetworkReference(
    PowerModels.build_ref(PowerModels.parse_file(filename))[:it][:pm][:nw][0]; kwargs...
)
# NetworkReference(filename::String; kwargs...) = NetworkReference(
...
# θ(ref, busvalue::Vector{JuMP.GenericAffExpr{Float64,JuMP.Variable}}, i) = foldl(append!, ref.π[i,j]*busvalue[j] for j in 1:ref.nbus)
